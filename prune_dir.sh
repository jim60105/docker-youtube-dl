...
then
    ...
    #
    find $DIR -type f -print0 | xargs -0 ls -tr | grep 'mp4$\|jpg$\|png$\|log$\|json$\|m4a$\|aac$\|flac$\|mp3$\|ogg$\|opus$\|wav$\|mka$\|mkv$\|mpg$\|ogv$\|ts$\|wma$\|wmv$\|webm$\|part$\|ytdl$' | while read FILE
    do
        if rm -f "$FILE"
        then
            echo "Deleted $FILE"
